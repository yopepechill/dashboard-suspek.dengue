...
function updateKPINasional() {
  ...
  const tahunFilter = document.getElementById("tahunDistribusi").value;

  let rows = dataNas;

  // ===== FILTER PROVINSI =====
  if (prov !== "INDONESIA") {
    rows = rows.filter(d => d.PROVINSI === prov);
  }

  // ===== FILTER TAHUN (INI YANG BARU 🔥) =====
  if (tahunFilter !== "ALL") {
    rows = rows.filter(d => String(d.Tahun).trim() === String(tahunFilter));
  }
  ...
  // ===== YoY =====
  document.getElementById("kpiYoY").innerHTML =
    tahunFilter === "ALL"
      ? "-"
      : hitungYoY(dataNas, tahunFilter);
}
...
function updateKPIJateng() {
  ...
  // rows untuk KPI utama (tahun terpilih)
  const rowsTahun = dataJateng.filter(d =>
    String(d.Tahun) === th
  );

  // rows untuk YoY (SEMUA tahun)
  const rowsAll = dataJateng;
  ...
  const kabTertinggi = Object.keys(mapKab).length
    ? Object.entries(mapKab).sort((a, b) => b[1] - a[1])[0][0]
    : "-";

  const avg = rowsTahun.length
    ? Math.round(total / rowsTahun.length)
    : 0;
  ...
  // ✅ YoY pakai semua tahun
  document.getElementById("kpiYoY").innerHTML =
    hitungYoY(dataJateng, th);
}
...
function updateBarSemarang() {
  ...
  // === SORT TERTINGGI ===
  const sorted = Object.entries(mapKec)
    .sort((a, b) => b[1] - a[1]);
    ...
    document.getElementById("barSemarang"),
    {
      ...
      data: {
        ...
        datasets: [{
          label: "Kasus Dengue",
          data: values,
          backgroundColor: "rgba(47, 171, 66, 0.6)",
          borderColor: "rgb(51, 176, 72)",
          borderWidth: 1
        }]
      },
      options: {
...
function loadPendudukSemarang() {
  ...
  // =========================
  // AGREGASI KASUS PER KECAMATAN
  // =========================
  const mapKec = {};

  dataFaskes.forEach(d => {
    if (String(d.Tahun).trim() !== String(tahun).trim()) return;
  ...
  const maxKasus = Math.max(...Object.values(mapKec), 0);

  // =========================
  // LOOP MARKER CENTROID
  // =========================
  Object.entries(mapKec).forEach(([kec, kasus]) => {
    ...
    const marker = L.circleMarker([center.lat, center.lng], {
      pane: 'paneMarker',
      radius: Math.max(6, Math.sqrt(kasus) / 2),
      fillColor: getColorMarker(kasus, maxKasus),
      color: "#222",
      weight: 1,
      fillOpacity: 0.8
    }).bindPopup(
  ...
  // ❗ jangan addTo — biar layerControl yang handle
}
...
function buatLayerControlSemarang() {
  ...
  layerControlSemarang = L.control.layers(
    null,
    {
      "Insidensi Kasus Kecamatan": layerPendudukSemarang,
      "Insidensi Faskes": layerFaskesSemarang
    },
    { collapsed: false }
  ).addTo(mapSemarang);
}
function tryInitSemarang() {
  ...
  layerFaskesSemarang.addTo(mapSemarang);
}
...
// HITUNG YoY
// =====================
function hitungYoY(rows, tahunAcuan) {

  if (!tahunAcuan) return "-";

  const thNow = String(tahunAcuan).trim();
  const thPrev = String(Number(thNow) - 1);

  let totalNow = 0;
  let totalPrev = 0;

  rows.forEach(d => {
    const th = String(d.Tahun).trim();
    const val = toNumber(d.Kasus);

    if (th === thNow) totalNow += val;
    if (th === thPrev) totalPrev += val;
  });
  ...
  const arrowHTML = naik
    ? `<span style="color:#d32f2f;font-weight:700">⬆</span>`
...
document.getElementById("tahunDistribusi").onchange = () => {
  updateDistribusi();
  updateKPINasional(); // 🔥 INI KUNCINYA
};
